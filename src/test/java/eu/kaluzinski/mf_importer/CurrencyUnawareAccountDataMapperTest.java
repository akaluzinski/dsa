...
class CurrencyUnawareAccountDataMapperTest {
  ...
  void shouldMapImportDataToAccountState() {
    ...
    //when
    var result = testedUnit.mapImportDataToAccountState("mBank", normalizedData);
  ...
  void shouldMapImportNoDataDataToUnknownAccountState() {
    ...
    //when
    var result = testedUnit.mapImportDataToAccountState("PKO", normalizedData);

    //then
    var expectedAccountState = new AccountState(EMPTY_LIST, EMPTY_LIST);
    assertEquals(expectedAccountState, result);
  }
